use super::super::{
    ...
    RenderNodeImplContext, RenderNodeImplContextInit, RenderViewportRect, ResourceAllocatorPhase,
...
struct TestFrameRuntime {
    ...
    active_pass: bool,
    calls: Vec<(RenderFlowGroup, RenderViewportRect)>,
}
...
fn viewport_outside_active_pass_fails_loudly() {
    let mut allocator = FrameResourceAllocator::new();
    let mut runtime = TestFrameRuntime::default();
    let viewport = RenderViewportRect::new(1, 2, 64, 32);
    let mut rctx = RenderNodeImplContext::new_with_runtime(
        &mut allocator,
        &mut runtime,
...
fn viewport_inside_active_pass_routes_through_frame_runtime() {
    ...
    let viewport = RenderViewportRect::new(0, 0, 128, 96);
    {
        let mut rctx = RenderNodeImplContext::new_with_runtime(
            &mut allocator,
            &mut runtime,
