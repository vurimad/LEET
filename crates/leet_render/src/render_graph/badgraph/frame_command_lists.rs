...
impl FrameCommandLists {
    ...
    pub fn validate_slot(&self, index: FrameCommandListIndex) -> LeetResult<()> {
        let state = self.lock_state()?;
        Self::slot(&state, index).map(|_| ())
    }
    ...
    fn lock_state(&self) -> LeetResult<std::sync::MutexGuard<'_, FrameCommandListsState>> {
        self.state
            .lock()
            .map_err(|_| Leeror::Unexpected("frame command-list registry was poisoned".to_string()))
    }
