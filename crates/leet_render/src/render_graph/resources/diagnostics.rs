...
use super::{
    AllocationRequestSource, FrameResourceAllocationClass, FrameResourceAllocator,
    FrameResourceDesc, FrameResourceKind, FrameResourceOwnership, FrameResourcePoolPlan,
    FrameResourceReuseRejectionReason, ImportedFrameResource, RenderFlowAutoId, RenderFlowSpace,
    RequestTime, ResourceRequest, TagLifetimeEventKind,
};
use std::fmt::Write;

pub struct FrameResourceDiagnostics<'a> {
    allocator: &'a FrameResourceAllocator,
}

impl<'a> FrameResourceDiagnostics<'a> {
    pub fn new(allocator: &'a FrameResourceAllocator) -> Self {
        Self { allocator }
    }
    ...
    pub fn dump_frame_summary(&self) -> String {
        ...
        let _ = writeln!(
            ...
            format_optional_time(self.allocator.current_consume_time())
        );
        let _ = writeln!(
            out,
            "  request_groups={}",
...
fn format_optional_time(time: Option<RequestTime>) -> String {
    time.map(format_time).unwrap_or_else(|| "none".to_string())
}
